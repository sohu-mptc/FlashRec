...
CLI::
    ...
    python -m flashrec.benchmark.recif --engine flashrec --server-url ... ...
"""
...
from flashrec.benchmark.utils import percentile, post_json
...
def unique_topk(beams: list[str], k: int) -> list[str]:
    """First ``k`` SIDs in rank order, skipping duplicates.

    Duplicate beams (path collapse) must not inflate recall/ndcg: the same
    SID counted 32× in top-32 yields recall=3.2 when ``|GT|=10``.
    """
    seen: set[str] = set()
    out: list[str] = []
    for sid in beams:
...
def ndcg_at_k(beams: list[str], gt: set[str], k: int) -> float:
    dcg = 0.0
    for i, sid in enumerate(unique_topk(beams, k), start=1):
        if sid in gt:
...
def sid_duplicate_rate(beams: list[str]) -> tuple[int, int, float]:
    """Return ``(n, n_unique, duplicate_rate)`` for one beam list.

    ``duplicate_rate = 1 - n_unique / n`` (0 = all distinct, 1 = fully collapsed).
    """
    n = len(beams)
    if n <= 0:
        return 0, 0, 0.0
    n_unique = len(set(beams))
    return n, n_unique, 1.0 - (n_unique / n)
...
def main() -> int:
    ...
    with (
        ...
        collapsed = 0  # unique == 1 (fully collapsed to one SID)
        for r in results:
            sids = [s for s, _ in r["beams"]]
            gt = r["gt"]
            n_cand, n_unique, dup_rate = sid_duplicate_rate(sids)
            dup_rates.append(dup_rate)
            unique_counts.append(n_unique)
            if n_cand > 0 and n_unique <= 1:
                collapsed += 1
            invalid_total += r["invalid"]
            cand_total += max(n_cand, 1)
            ...
            prow = [
                ...
                n_unique,
                round(dup_rate, 6),
                round(r["invalid"] / max(n_cand, 1), 6),
                r["gt_count"],
                round(r["latency"], 6),
            ]
            for k in ks:
    ...
    mean_dup = sum(dup_rates) / n if n else 0.0
    mean_unique = sum(unique_counts) / n if n else 0.0
    summary = {
        "engine": args.engine,
        ...
        "latency_p90_s": round(percentile(latencies, 90), 6),
        "latency_p95_s": round(percentile(latencies, 95), 6),
        "latency_p99_s": round(percentile(latencies, 99), 6),
        "invalid_rate": round(invalid_total / max(cand_total, 1), 6),
        "duplicate_rate": round(mean_dup, 6),
        "mean_unique_candidates": round(mean_unique, 3),
        "collapsed_rate": round(collapsed / n, 6) if n else 0.0,
    ...
    print(
        "[done] {engine} n={n} c={c} T={T} samples={s} wall={w:.1f}s qps={q:.3f} "
        "recall@32={r:.5f} ndcg@32={d:.5f} invalid={inv:.4f} "
        "dup={dup:.4f} uniq={uniq:.1f} collapsed={col:.4f} "
        "p50={p50:.3f}s p90={p90:.3f}s p99={p99:.3f}s".format(
            engine=args.engine,
            n=args.n,
            c=args.concurrency,
            T=args.temperature,
            ...
            dup=summary["duplicate_rate"],
            uniq=summary["mean_unique_candidates"],
            col=summary["collapsed_rate"],
            p50=summary["latency_p50_s"],
            p90=summary["latency_p90_s"],
            p99=summary["latency_p99_s"],
        ),
        flush=True,
    )
    return 0
