...
#   SKIP_LAUNCH=1                       # eval against an already-running server
#
# Writes results/onerec_beam_conc_bench_<stamp>/{MATRIX_REPORT.md,matrix_summary.json}.
# Compatibility shim: bash scripts/bench_sglang_compare.sh
#
set -uo pipefail
...
_flashrec_maybe_loop_models "$ROOT/benchmark/recif/bench_compare.sh"
...
if [[ "${SMOKE:-0}" == "1" ]]; then
  OUTDIR="${OUTDIR:-$ROOT/results/onerec_beam_conc_bench_smoke_${MODEL_TAG}_$TS}"
else
  OUTDIR="${OUTDIR:-$ROOT/results/onerec_beam_conc_bench_${MODEL_TAG}_$TS}"
fi
...
start_flashrec() {
  ...
  # Optional FlashRec CLI knobs, e.g.:
  #   EXTRA_FLASHREC_ARGS='--enable-cascade-attention --cascade-min-rows 128'
  # shellcheck disable=SC2206
  local extra_fr=()
  if [[ -n "${EXTRA_FLASHREC_ARGS:-}" ]]; then
    # intentional word-split of operator-supplied args
